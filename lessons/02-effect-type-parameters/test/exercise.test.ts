...
import {
  ...
  type OrderQuote,
  provideDiscountPolicy,
  type QuantityTooLarge,
  quoteOrder
...
describe("Lesson 02 exercise: reading Effect<A, E, R>", () => {
  ...
  it("provides DiscountPolicy so the runnable program has no R requirement", () => {
    ...
    const runnable: Effect.Effect<OrderQuote, InvalidQuantity | QuantityTooLarge, never> =
      provideDiscountPolicy(program, {
        unitPriceCents: 100,
        maxQuantity: 10,
        discountPercent: 10
      })

    expect(Effect.runSync(runnable).quantity).toBe(5)
  })
})
